...
function getRestaurantInfo($a) {
    ...
    try {
        ...
        if ($a=='最高評分') {
           ...
           $sql = "SELECT * FROM RestaurantCategory WHERE category_name = ?";
           $stmt = $db->prepare($sql);
           $stmt->execute([$a]);
...
function getSaleInfo($a) {
    ...
    try {
        ...
        // 如果不是的話 取該分類的限時優惠
        if($a=='最高評分'){
            $sql = "SELECT * FROM RestaurantCategory where is_onSale = 1  order by rating desc";
            $stmt = $db->prepare($sql);
            $stmt->execute();

        }else{
            $sql = "SELECT * FROM RestaurantCategory where category_name = ? and  is_onSale = 1";
            $stmt = $db->prepare($sql);
            $stmt->execute([$a]); 
        }
